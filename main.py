...
#video_path = r"D:\Work\NCKH\NCKH\falltest5.mp4"
video_path = 0
...
# Giới hạn số lượng điểm dữ liệu bạn muốn hiển thị trên biểu đồmax_data_points = 100
...
# Khởi tạo đường dẫn cho tệp tin cảnh báo
alert_log_path = os.path.join(output_folder, "alert_log.txt")

# Biến flag để kiểm tra xem đã ghi vào log file fall detect chưa
fall_detect_logged = False
...
async def process_frames():
    ...
    global video_writer
    global fall_detect_logged  # Add this global variable
    ...
    while True:
        ...
        if len(motion_pixels_list) > max_data_points:
            ...
            time_list.pop(0)  # Sử dụng dữ liệu có trong time_list và motion_pixels_list để giới hạn khung hình biểu diễn trong đồ thị điểm ảnh

        big_frame = cv2.resize(img_display, (img_original.shape[1], img_original.shape[0]))  # Gom tất cả các khung hình rời rạc lại thành một khung hình chung "Big Frame"
        ...
        if bboxInfo:
            ...
            if length_horizontal > 0:
                ...
                if alert:
                    print("Fall Detect")
                    if not fall_detect_logged:
                        print("Fall Detect")
                        with open(alert_log_path, 'a') as alert_log_file:
                            alert_log_file.write(f"{datetime.now()} - Fall Detect\n")
                        fall_detect_logged = True
